...
function showInfo(){
    for (var i = 0; i < size; i++) {
        var sprice = document.getElementById("input"+i).value;
      
        var price = sprice.toLocaleString('en-IN', {
            maximumFractionDigits: 2,
            currency: 'INR'
        });

        var pod = document.getElementById("pod"+i).value;
        $("#price"+pod).text(price);

        var name = document.getElementById("iname"+i).value;
        $("#name"+pod).text(name);
    }
}
